...
# --- FILTRI GLOBALI SEMPRE DEFINITI, CON CHIAVE UNICA ---
...
st.sidebar.markdown("""
1. Imposta l'intervallo di date desiderato per l'analisi (di default da oggi a 30 giorni).
2. Clicca su 'Scarica turnazioni da Teams per tutti i negozi'.
3. L'applicazione scaricherà i dati, eseguirà le analisi e mostrerà i risultati per tutti i negozi.
""")

st.sidebar.header("Scarica turnazioni da Teams Shifts (Microsoft Graph API)")
client_id = os.getenv("AZURE_CLIENT_ID")
client_secret = os.getenv("AZURE_CLIENT_SECRET")
tenant_id = os.getenv("AZURE_TENANT_ID")
scarica_teams = st.sidebar.button("Scarica turnazioni da Teams per tutti i negozi")

if scarica_teams:
    if not (client_id and client_secret and tenant_id):
        st.sidebar.error("Le credenziali Azure (Client ID, Client Secret, Tenant ID) non sono state trovate. Assicurati di averle impostate nel file .env.")
    else:
        with st.spinner("Scarico i dati da Teams Shifts per tutti i negozi..."):
            ...
            if "access_token" in result:
                ...
                # Carica il mapping negozio -> team_id
                try:
                    with open('team_mapping.json', 'r') as f:
                        team_mapping = json.load(f)
                except Exception as e:
                    st.error(f"Errore nel caricamento di team_mapping.json: {e}")
                    team_mapping = {}
                ...
                for idx, negozio in enumerate(negozi_lista):
                    ...
                    try:
                        ...
                        if response_schedule.status_code == 200:
                            ...
                            st.warning(f"Impossibile recuperare il fuso orario per {negozio}. Verrà utilizzato UTC.")
                            negozio_timezones[negozio] = "UTC"
                    except requests.exceptions.RequestException as e:
                        st.warning(f"Errore di connessione durante il recupero del fuso orario per {negozio}: {e}. Verrà utilizzato UTC.")
                        negozio_timezones[negozio] = "UTC"
                    ...
                    while url_shifts:
                        ...
                        if response_shifts.status_code == 200:
                            ...
                            st.warning(f"Errore nel recupero turni per {negozio}: {response_shifts.text}")
                            break
                    progress_bar.progress((idx + 1) / len(negozi_lista), text=f"{negozio} completato")
                progress_bar.empty()
                if all_shifts:
                    ...
                    # Filtro sulle date e user_id non vuoto
                    ...
                    if df_shifts.empty:
                        st.warning('Nessun dato disponibile per l\'intervallo selezionato e/o user_id non vuoti.')
                    else:
                        # Aggregazione per store e data (solo data, formato stringa YYYY-MM-DD)
                        ...
                        st.header('Analisi automatica: Persone programmate vs Vendite per negozio')
                        st.success("Dati turnazioni scaricati con successo!")
                        st.write("Anteprima dei dati scaricati:")
...
if 'schedule_df' in st.session_state and not st.session_state['schedule_df'].empty:
    ...
    for negozio in schedule_df['negozio'].unique():
        ...
        if vendite_negozio.empty:
            st.warning(f"Nessun dato di vendita per il negozio {negozio}")
            continue
            ...
            if lat != 0 and lon != 0 and not vendite_2024.empty:
                ...
                try:
                    ...
                    if weather_response.status_code == 200:
                        ...
                        st.warning(f"Non è stato possibile recuperare il meteo per {negozio}. (API Status: {weather_response.status_code})")
                except requests.exceptions.RequestException as e:
                    st.warning(f"Errore di connessione durante il recupero del meteo per {negozio}: {e}")

        except (FileNotFoundError, KeyError):
            st.warning(f"Nessuna coordinata trovata per {negozio} in 'store_locations.json' o file non trovato.")
        ...
        st.subheader(f"{negozio}: Persone programmate e media vendite storiche per combinazione")
        result['data_label'] = result['data_dt'].dt.strftime('%Y-%m-%d') + ' (' + result['giorno_settimana'] + ')'
        fig = go.Figure()
        fig.add_trace(go.Bar(x=result['data_label'], y=result['vendite'], name='Media vendite storiche 2024', marker_color='blue'))
        ...
        fig.add_trace(go.Scatter(x=result['data_label'], y=result['num_persone'], name='Persone programmate 2025', mode='lines+markers', marker_color='orange', yaxis='y2'))
        if attenzione_idx:
            fig.add_trace(go.Scatter(
                ...
                name='Punto di attenzione',
                yaxis='y2',
                showlegend=True
            ))
        fig.update_layout(
            title=f"{negozio}: Persone programmate e media vendite storiche per combinazione",
            xaxis_title="Data (Giorno della settimana)",
            yaxis_title="Media vendite storiche 2024",
            yaxis2=dict(title="Persone programmate 2025", overlaying='y', side='right', tickformat=',d', dtick=1),
        ...
        if coverage_df is not None:
            st.subheader(f"Analisi Copertura Oraria per {negozio}")
            ...
            if not negozio_coverage.empty:
                ...
                st.write("Nessun dato di copertura oraria disponibile per questo negozio.") 
